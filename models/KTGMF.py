...
class KTGMF(nn.Module):
    def __init__(self, node_raw_features: np.ndarray,
        ...
        self.projection_layer = nn.ModuleDict({
            'feature_Linear': nn.Linear(in_features=self.node_raw_features.shape[-1], out_features=self.node_dim,
                                        bias=True),
        ...
        self.src_node_updater = KTGMF_Seq(edge_dim=self.edge_dim, node_dim=self.node_dim)
        self.dst_node_updater = KTGMF_Seq(edge_dim=self.edge_dim, node_dim=self.node_dim)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, edge_ids: np.ndarray,
        ...
        src_nodes_neighbor_co_occurrence_features = (
                    torch.from_numpy(src_neighbor_node_ids[:, :-1]) == torch.from_numpy(dst_node_ids).unsqueeze(
                1).repeat(1, self.num_neighbors)).unsqueeze(-1).float().to(self.device)
        dst_nodes_neighbor_co_occurrence_features = (
                    torch.from_numpy(dst_neighbor_node_ids[:, :-1]) == torch.from_numpy(src_node_ids).unsqueeze(
                1).repeat(1, self.num_neighbors)).unsqueeze(-1).float().to(self.device)

        src_node_skill = self.node_raw_features[torch.from_numpy(src_neighbor_node_ids)][:, :-1, 0].long().to(
            self.device)
        dst_node_skill = self.node_raw_features[torch.from_numpy(dst_neighbor_node_ids)][:, -1, 0].long().to(
            self.device).unsqueeze(1).repeat(1, self.num_neighbors)
        ...
        src_node_embeddings = self.src_node_updater.update(
            ...
            :] + src_nodes_neighbor_skill_struct_features + src_nodes_neighbor_struct_features) + (
        ...
        if self.ablation in ['q_qid', 'q_kid']:
            ...
            dst_node_embeddings = self.dst_node_updater.update(
                ...
                :] + dst_nodes_neighbor_struct_features) + dst_nodes_features[:, -1, :]
    ...
    def get_features(self, nodes_neighbor_ids: np.ndarray, nodes_edge_ids: np.ndarray,
        ...
        nodes_edge_raw_features = self.projection_layer['edge'](
            self.edge_raw_features[torch.from_numpy(nodes_edge_ids)].to(self.device)[:, :, 0].unsqueeze(-1))
...
class KTGMF_Seq(nn.Module):

    def __init__(self, edge_dim: int, node_dim: int):

        super(KTGMF_Seq, self).__init__()
        ...
        self.hid_node_updater = SRU(input_size=edge_dim, hidden_size=node_dim,num_layers=2, bidirectional=True)
