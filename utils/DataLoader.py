...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    edge_raw_features = np.load('./processed_data/{}/ml_{}.npy'.format(dataset_name, dataset_name),allow_pickle=True)
    node_raw_features = np.load('./processed_data/{}/ml_{}_node.npy'.format(dataset_name, dataset_name),allow_pickle=True)
    # edge_raw_features = np.tile(edge_raw_features, (1, 4)).reshape(-1, 8)
    NODE_FEAT_DIM = EDGE_FEAT_DIM = 256
    ...
    #print("test node set",len(test_node_set))
...
def get_link_classification_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    graph_df = pd.read_csv('./processed_data/{}/ml_{}.csv'.format(dataset_name, dataset_name))
    #边的权重是做题是否作对，作为边的特征
    edge_raw_features = np.load('./processed_data/{}/ml_{}.npy'.format(dataset_name, dataset_name),allow_pickle=True)[:,:4]
    #点的权重一开始只有i有，不知道哪儿来的
    node_raw_features = np.load('./processed_data/{}/ml_{}_node.npy'.format(dataset_name, dataset_name),allow_pickle=True)[:,:4]
    # edge_raw_features = np.tile(edge_raw_features, (1, 4)).reshape(-1, 8)
    NODE_FEAT_DIM = EDGE_FEAT_DIM = 64#128
    print(graph_df.head(2))#.shape,node_raw_features.shape[0], edge_raw_features.shape[0])
    assert NODE_FEAT_DIM >= node_raw_features.shape[1], f'Node feature dimension in dataset {dataset_name} is bigger than {NODE_FEAT_DIM}!'
    assert EDGE_FEAT_DIM >= edge_raw_features.shape[1], f'Edge feature dimension in dataset {dataset_name} is bigger than {EDGE_FEAT_DIM}!'
    # padding the features of edges and nodes to the same dimension (172 for all the datasets)
    if node_raw_features.shape[1] < NODE_FEAT_DIM:
        node_zero_padding = np.zeros((node_raw_features.shape[0], NODE_FEAT_DIM - node_raw_features.shape[1]))
        node_raw_features = np.concatenate([node_raw_features, node_zero_padding], axis=1)
        #点构成12910*64的矩阵，第一列是原数据
    if edge_raw_features.shape[1] < EDGE_FEAT_DIM:
        edge_zero_padding = np.zeros((edge_raw_features.shape[0], EDGE_FEAT_DIM - edge_raw_features.shape[1]))
        edge_raw_features = np.concatenate([edge_raw_features, edge_zero_padding], axis=1)
        #边构成941556*64的矩阵，第一列是原数据/做题正确与否

    assert NODE_FEAT_DIM == node_raw_features.shape[1] and EDGE_FEAT_DIM == edge_raw_features.shape[1], 'Unaligned feature dimensions after feature padding!'

    # get the timestamp of validate and test set
    val_time, test_time = list(np.quantile(graph_df.ts, [(1 - val_ratio - test_ratio), (1 - test_ratio)]))
    #验证集与测试集的开始时间，训练集的开始时间是0
    src_node_ids = graph_df.u.values.astype(np.longlong)
    dst_node_ids = graph_df.i.values.astype(np.longlong)
    node_interact_times = graph_df.ts.values.astype(np.float64)
    edge_ids = graph_df.idx.values.astype(np.longlong)
    labels = graph_df.label.values
    #分别是数据集第二列u,第三列i,第四列ts,第六列index,第五列正确与否
    ...
    #print("test node set",len(test_node_set))
    # sample nodes which we keep as new nodes (to test inductiveness), so then we have to remove all their edges from training
    # print(len(test_node_set), num_total_unique_node_ids)
    new_test_node_set = set(random.sample(sorted(test_node_set), int(0.1 * num_total_unique_node_ids)))
